...
public class FriendshipWorkload extends Workload {
	...
	// {
	// res = db.inviteFriend(dbKey, keyNum); //data = new StringByteIterator("1");
	// // Pending.
	// }else {
	// res = db.CreateFriendship(dbKey, keyNum); //data = new
	// StringByteIterator("2"); // Already.
	// }
	...
	public boolean doInsert(DB db, Object threadState) {
		...
		if (feedLoad) {// for pending friends and conf=0 may not get a uniform number of pending
						// requests per user
			// popular users in every cluster are friends with popular users (except for the
			// beginning and end of the clusters)
			// the avergae count is the max number of friend sfor a user and not all users
			// may have the
