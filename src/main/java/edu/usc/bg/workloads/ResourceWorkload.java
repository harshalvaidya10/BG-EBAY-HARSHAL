...
public class ResourceWorkload extends Workload {
	...
	public void init(Properties p, Vector<Integer> members) throws WorkloadException {
		...
		System.out.println(
				"ResourceWorkload init: userCount=" + userCount +
						", avgResourceCount=" + avgResourceCount +
						", recordCount=" + recordCount +
						", creatorSequence=" + creatorSequence +
						", keyIdx=" + keyIdx +
						", _members.size=" + (_members != null ? _members.size() : "null"));
		System.out.println("Exit init method of ResourceWorkload");
		return;
	}
	...
	private LinkedHashMap<String, ByteIterator> buildValues(String dbKey, int creatorNum) {
		...
		for (int i = 1; i <= fieldCount; ++i) {
			...
			if (1 == i) {
				...
				data = new ObjectByteIterator(Integer.toString(_members.get(random.nextInt(userCount))).getBytes()); // WallUser
																														// ID.
			} else if (6 == i) {
